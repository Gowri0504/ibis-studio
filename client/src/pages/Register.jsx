    ...
    <div className="min-h-screen pt-32 pb-20 bg-ibis-dark text-white">
      <div className="container mx-auto px-6 max-w-md">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="glass-card p-8 rounded-2xl border border-white/10">
          <h1 className="text-3xl font-serif font-bold mb-6">Create Account</h1>
          <form onSubmit={onSubmit} className="space-y-6">
            <div>
              <label className="block text-sm text-gray-400 mb-2">Name</label>
              <input name="name" value={form.name} onChange={onChange} className="w-full bg-black/40 border border-gray-700 rounded-lg p-3 text-white" required />
            </div>
            <div>
            ...
            <div>
              ...
              <input name="phone" value={form.phone} onChange={onChange} className="w-full bg-black/40 border border-gray-700 rounded-lg p-3 text-white" required />
            </div>
            <div>
            ...
            <button type="submit" disabled={loading} className="w-full btn-gold-shine text-black font-bold py-3 rounded-lg">{loading ? 'Creating...' : 'Register'}</button>
          </form>
        </motion.div>
      </div>
      {alertMsg && (
